...
class BinConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True, istrain=True, drop=0):
        super(BinConv2d,self).__init__(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias)
...
class BinLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, istrain=True, drop=0):
        super(BinLinear,self).__init__(in_features, out_features, bias)
...
class binop_train:
    ...
    def binarization(self):
        for index in range(self.num_of_params):
            ...
            self.target_modules[index].data = self.target_modules[index].data.sign().mul(self.alpha_to_save[index].data.expand(s))
