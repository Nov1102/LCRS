...
def test(model, test_loader, num_branches):
    model.eval()
    model_losses = [0]*(num_branches + 1)
    num_correct = [0]*(num_branches + 1)
...
if __name__ == '__main__':
    # Training settings
    parser = argparse.ArgumentParser(description='LCRS')
    ...
    model = net.LCRS(in_channels, 10, 1)
    if args.cuda:
        model = model.cuda()
